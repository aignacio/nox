...
class nox(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","nox_sim")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for nox start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' -s 800000000 -e {2} --isa={0} +signature={1} +signature-granularity=4'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
      ...
      # make.execute_all(self.work_dir, timeout=10000)
      make.execute_all(self.work_dir, timeout=300)
      # make.execute_all(self.work_dir, timeout=10)
